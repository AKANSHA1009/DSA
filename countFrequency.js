...
function countFrequencyBruteForce(arr) {
    let counted = [];  // Array to keep track of the elements we've already counted
    
    // Loop through each element of the array
    for (let i = 0; i < arr.length; i++) {
        let currentElement = arr[i];
        
        // If the element is not already counted, count its frequency
        if (!counted.includes(currentElement)) {
            ...
            // Add the current element to the counted array
